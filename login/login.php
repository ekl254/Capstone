    ...
    <div class="row">
      <div class="col-md-4">
        <center>
          ...
          <form action='' method = 'post' onsubmit="computeHashed()";>
          ...
          if(isset($_POST['submit'])){
            $password = $data[$_POST["username"]]["password"];
            $results = hash('sha256', $password.$_SESSION['nonce']);
            if($password == ""){
              echo "\nInvalid Login!!!";
              $_SESSION["current"] = "";
            }
            else if($results == $_POST["password"]){
              echo "Logged in!";
          ...
          ?>
          <br>
          <p> Need access? <a href="https://www.youtube.com/watch?v=dQw4w9WgXcQ">Request Account</a></p>
        </div>
      </center>
    </div>
    <br>
